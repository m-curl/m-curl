...
class CTMR(nn.Module):
    ...
    def encode_obs(self, obs, detach=False, ema=False):
        ...
        if ema:
            with torch.no_grad():
                z_out = self.encoder_target(x, project=self.project)
        else:
            z_out = self.encoder(x, project=self.project)
    ...
    def compute_logits(self, z_a, z_pos, ):
        if self.normalize:
            z_a = z_a / torch.norm(z_a, dim=-1, keepdim=True)
            z_pos = z_pos / torch.norm(z_pos, dim=-1, keepdim=True)
...
class CtmrSacAgent(object):
    ...
    def __init__(self,
        ...
        if self.encoder_type == 'pixel':
            self.CTMR = CTMR(encoder_feature_dim,  self.critic, self.critic_target,
                             ...
                             dropout=dropout, attention_dropout=attention_dropout,
                             relu_dropout=relu_dropout, normalize=normalize, tau=tau,
                             no_project=no_project).to(self.device)
    ...
    def update_cpc(self, cpc_kwargs, L, step):
        ...
        loss.backward()
        nn.utils.clip_grad_norm_(self.CTMR.parameters(), self.max_grad_norm)
