...
def plot_data(data, xaxis='timesteps', value="Performance", condition="Condition1", smooth=1, **kwargs):
    ...
    g = sns.tsplot(data=data, time=xaxis, value=value, unit="Unit", condition=condition, ci='sd',)
    plt.xlabel(kwargs['xlabel'])
    plt.ylabel(kwargs['ylabel'])
    if kwargs['enable_legend']:
        plt.legend(loc='best', fontsize=26).set_draggable(True)
    else:
        g.legend_.remove()
    ...
    if not kwargs['disable_vline']:
        axis.axvline(100000, color='m', lw=1)
        axis.axvline(500000, color='c', lw=1)
...
def changeargs(args):
    ...
    if env_task in ['hopper-hop', 'pendulum-swingup']:
        args.timesteps = 4e6
    if env_task in ['finger-spin', 'walker-walk']:
...
def main():
    ...
    parser.add_argument('--enable-legend', action='store_true', default=False)
